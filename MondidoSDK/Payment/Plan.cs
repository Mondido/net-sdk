...
namespace Mondido.Payment
{
    public class Plan : BaseModel
    {
        ...
        {
            return HttpPost("/plans", data).Result.FromJson<Plan>();
        }

        public static Plan Get(int id)
        {
            return HttpGet("/plans/" + id).Result.FromJson<Plan>();
        }
        ...
        public static IEnumerable<Plan> List(int take, int skip, List<KeyValuePair<string, string>> filters = null, string sortBy = "id:desc")
        {
            ...
            return HttpGet(string.Format("/plans?limit={0}&offset={1}{3}&order_by={2}", take, skip, sortBy, parsedFilters)).Result.FromJson<IEnumerable<Plan>>();
        }

        public static Plan Delete(int id)
        {
            return HttpDelete(string.Format("/plans/" + id)).Result.FromJson<Plan>();
        }
