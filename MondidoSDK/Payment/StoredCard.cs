...
namespace Mondido.Payment
{
    public class StoredCard : BaseModel
    {
        ...
        {
            return HttpPost("/stored_cards", data).FromJson<StoredCard>();
        }

        public static StoredCard Get(int id)
        {
            return HttpGet("/stored_cards/" + id).FromJson<StoredCard>();
        }
        ...
        public static IEnumerable<StoredCard> List(int take, int skip, List<KeyValuePair<string, string>> filters = null, string sortBy = "id:desc")
        {
            ...
            return HttpGet(string.Format("/stored_cards?limit={0}&offset={1}{3}&order_by={2}", take, skip, sortBy, parsedFilters)).FromJson<IEnumerable<StoredCard>>();
        }

        public static StoredCard Delete(int id)
        {
            return HttpDelete(string.Format("/stored_cards/" + id)).FromJson<StoredCard>();
        }

        public static StoredCard Update(int id, List<KeyValuePair<string, string>> data)
        {
            return HttpPut(string.Format("/stored_cards/" + id),data).FromJson<StoredCard>();
        }
    }
}